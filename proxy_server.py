...
def get_allowed_client_networks() -> list[ipaddress._BaseNetwork]:
    """Return allowed client CIDR ranges for the local proxy.

    LOCAL_PROXY_ALLOWED_IPS accepts comma-separated IPs/CIDRs.
    Default is loopback-only, which is safe even if the proxy bind host is changed
    to 0.0.0.0/::. Add your home/office IP explicitly when exposing the proxy.
    """
    raw = os.environ.get("LOCAL_PROXY_ALLOWED_IPS") or os.environ.get("LOCAL_PROXY_WHITELIST")
    entries = _split_csv(raw) or ["127.0.0.1/32", "::1/128"]
    ...
    for entry in entries:
        try:
            ...
            try:
                ...
                networks.append(ipaddress.ip_network(f"{addr}/32" if addr.version == 4 else f"{addr}/128", strict=False))
            except ValueError:
                print(f"[代理访问控制] 忽略无效白名单条目: {entry}", flush=True)
    return networks or [ipaddress.ip_network("127.0.0.1/32"), ipaddress.ip_network("::1/128")]

def client_ip_allowed(address: tuple[str, int]) -> bool:
    ...
    try:
        ...
        return False
    return any(ip in net for net in get_allowed_client_networks())
...
def start_proxy_server(host: str, port: int) -> None:
    ...
    while True:
        try:
            ...
            if not client_ip_allowed(address):
                print(f"[代理访问控制] 拒绝未授权客户端 {address[0]}。请在 LOCAL_PROXY_ALLOWED_IPS 中加入允许的公网 IP/CIDR。", flush=True)
                try:
                    client.close()
                except OSError:
                    pass
