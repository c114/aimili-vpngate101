...
# 5. Configure Service
echo -e "\n${YELLOW}[3/4] 正在配置系统服务与私有代理安全参数...${PLAIN}"

ENV_FILE="/etc/default/aimilivpn"
if [ ! -f "$ENV_FILE" ]; then
    PROXY_USER="user_$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 8)"
    PROXY_PASS="$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 18)"
    cat > "$ENV_FILE" <<EOF
# Aimili private gateway runtime configuration
# 默认只监听本机，推荐通过 SSH 隧道使用：ssh -L 7928:127.0.0.1:7928 root@你的VPS
LOCAL_PROXY_HOST=127.0.0.1
LOCAL_PROXY_ALLOWED_IPS=127.0.0.1/32,::1/128
LOCAL_PROXY_USER=$PROXY_USER
LOCAL_PROXY_PASS=$PROXY_PASS
LOCAL_PROXY_MAX_CONNECTIONS=64
EOF
    chmod 600 "$ENV_FILE"
    echo -e "${GREEN}  -> 已创建 $ENV_FILE，默认仅允许本机/SSH 隧道访问代理。${PLAIN}"
    echo -e "${YELLOW}  -> 代理认证账号: $PROXY_USER${PLAIN}"
    echo -e "${YELLOW}  -> 代理认证密码: $PROXY_PASS${PLAIN}"
else
    echo -e "${GREEN}  -> 检测到 $ENV_FILE，保留现有私有代理安全参数。${PLAIN}"
fi
...
def load_ui_cfg():
    ...
    cfg = {"host": "::", "port": 8787, "secret_path": "EJsW2EeBo9lY", "password": "", "password_hash": ""}
...
def print_status():
    ...
    if curr_pwd:
        ...
        masked_pwd = "已哈希保存；如忘记请用 ml password 重置"
    else:
        masked_pwd = "未配置"
...
def get_status_state():
    ...
    return (
        ...
        cfg.get("password", "") or ("已哈希保存，请用 ml password 重置" if cfg.get("password_hash") else ""),
...
if [ -f "$AUTH_FILE" ]; then
    ...
    PASSWORD=$(python3 -c "import json; d=json.load(open('$AUTH_FILE')); print(d.get('password') or ('已哈希保存，请用 ml password 重置' if d.get('password_hash') else '未配置'))" 2>/dev/null || echo "未配置")
