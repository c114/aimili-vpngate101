...
UI_LOGIN_MAX_ATTEMPTS = env_int("UI_LOGIN_MAX_ATTEMPTS", 8, 1, 1000)
UI_LOGIN_WINDOW_SECONDS = env_int("UI_LOGIN_WINDOW_SECONDS", 15 * 60, 30)
UI_LOGIN_LOCK_SECONDS = env_int("UI_LOGIN_LOCK_SECONDS", 10 * 60, 30)
...
def subscription_token_valid(token: str | None) -> bool:
    expected = str(load_ui_config().get("subscription_token") or "")
    return bool(expected and token and secrets.compare_digest(str(token), expected))

def public_proxy_host(headers: Any, query: dict[str, list[str]]) -> str:
    explicit = (query.get("host") or [""])[0].strip()
    if explicit:
        return explicit
    forwarded = str(headers.get("X-Forwarded-Host") or "").split(",")[0].strip()
    host_header = forwarded or str(headers.get("Host") or "").strip()
    ...
    if not host:
        host = "127.0.0.1"
    # When using SSH tunnel, Clash on the local machine should connect to localhost.
    if host in ("0.0.0.0", "::", ""):
        host = "127.0.0.1"
    return host

def build_clash_subscription(headers: Any, query: dict[str, list[str]]) -> str:
    host = public_proxy_host(headers, query)
    port = int(load_ui_config().get("proxy_port") or LOCAL_PROXY_PORT)
...
def subscription_urls(headers: Any) -> dict[str, str]:
    cfg = load_ui_config()
    token = cfg.get("subscription_token") or ""
    secret_path = cfg.get("secret_path") or ""
    host = str(headers.get("Host") or f"127.0.0.1:{cfg.get('port', UI_PORT)}")
    prefix = f"/{secret_path}" if secret_path else ""
    return {
        "clash": f"http://{host}{prefix}/sub/clash?token={urllib.parse.quote(str(token))}",
        "token": str(token),
    }
...
def save_ui_config(config: dict[str, Any]) -> None:
    """Persist UI/auth configuration with restrictive file permissions."""
    with lock:
        DATA_DIR.mkdir(exist_ok=True, parents=True)
...
def hash_ui_password(password: str, salt: str | None = None) -> str:
...
def verify_ui_password(password: str, ui_cfg: dict[str, Any]) -> bool:
    stored_hash = str(ui_cfg.get("password_hash") or "")
    if stored_hash:
        try:
            ...
            return False
    legacy_password = str(ui_cfg.get("password") or "")
    return bool(legacy_password) and secrets.compare_digest(password, legacy_password)


def set_ui_password(ui_cfg: dict[str, Any], password: str) -> None:
    ui_cfg["password_hash"] = hash_ui_password(password)
    ui_cfg.pop("password", None)


def migrate_legacy_password_if_needed(ui_cfg: dict[str, Any], password: str) -> None:
    if ui_cfg.get("password_hash") or not ui_cfg.get("password"):
        return
    if secrets.compare_digest(str(ui_cfg.get("password") or ""), password):
        set_ui_password(ui_cfg, password)
        save_ui_config(ui_cfg)
        print("[Auth] 已将网页登录密码迁移为 PBKDF2 哈希存储。", flush=True)
...
def login_locked_seconds(client_ip: str) -> int:
    now = time.time()
    with lock:
...
def record_login_failure(client_ip: str) -> int:
    now = time.time()
    window_start = now - UI_LOGIN_WINDOW_SECONDS
    with lock:
        entry = login_failures.setdefault(client_ip, {"attempts": [], "locked_until": 0.0})
        attempts = [float(t) for t in entry.get("attempts", []) if float(t) >= window_start]
        attempts.append(now)
        entry["attempts"] = attempts
        if len(attempts) >= UI_LOGIN_MAX_ATTEMPTS:
            entry["locked_until"] = now + UI_LOGIN_LOCK_SECONDS
            entry["attempts"] = []
            return UI_LOGIN_LOCK_SECONDS
        return 0
...
DANGEROUS_OVPN_DIRECTIVES = {
    ...
    "learn-address", "client-connect", "client-disconnect", "auth-user-pass-verify",
    "tls-verify", "plugin", "iproute", "management", "management-client",
    "management-query-passwords", "management-hold", "tmp-dir", "writepid", "log",
    "log-append", "status", "cd", "chroot", "askpass",
}
FILE_OVPN_DIRECTIVES = {
    "ca", "cert", "key", "pkcs12", "secret", "tls-auth", "tls-crypt",
    "crl-verify", "dh", "extra-certs",
}
ALLOWED_INLINE_BLOCKS = {
    "ca", "cert", "key", "tls-auth", "tls-crypt", "dh", "extra-certs",
}


def _ovpn_directive(line: str) -> tuple[str, list[str]]:
    try:
...
def sanitize_openvpn_config(config_text: str) -> str:
    """Remove OpenVPN directives that can execute scripts or read/write local files.

    VPNGate profiles are remote data. The manager still passes its own safe runtime
    options on the command line, so filtering these directives keeps deployment safer
    without removing the connection parameters and inline certificates VPNGate needs.
    """
    cleaned: list[str] = []
    skipped: list[str] = []
    ...
    for raw_line in (config_text or "").splitlines():
        ...
        block_match = re.match(r"^<\s*([A-Za-z0-9_-]+)\s*>$", stripped)
        if block_match:
            block_name = block_match.group(1).lower()
            if block_name in ALLOWED_INLINE_BLOCKS:
                in_block = block_name
                cleaned.append(raw_line)
            else:
                skip_block = block_name
                skipped.append(f"<{block_name}>")
            continue

        directive, parts = _ovpn_directive(stripped)
        if directive in DANGEROUS_OVPN_DIRECTIVES:
            skipped.append(directive)
            continue
        if directive in FILE_OVPN_DIRECTIVES and len(parts) > 1:
            skipped.append(directive)
            continue
    ...
    if skipped:
        ...
        print(f"[OpenVPN 配置过滤] 已移除不安全或本地文件相关指令: {unique}", flush=True)
    return "\n".join(cleaned).rstrip() + "\n"
...
def load_ui_config() -> dict[str, Any]:
    with lock:
        ...
        if auth_file.exists():
            try:
                ...
                for key, val in data.items():
                    config[key] = val
                for key in ["host", "port", "proxy_port", "routing_mode", "force_country", "routing_ip_type", "connection_enabled", "fixed_node_id", "favorite_node_ids", "fav_fail_fallback", "subscription_token", "password_hash"]:
                    if key not in data:
                        updated = True
            except Exception:
                pass
        ...
        if not config.get("password") and not config.get("password_hash"):
            # Store the initial generated password in legacy plaintext so the installer
            # and `ml` command can show it once. It is migrated to a PBKDF2 hash after
            # the first successful web login or when changed in the UI.
...
def get_state() -> dict[str, Any]:
    ...
    state["password_set"] = bool(ui_cfg.get("password") or ui_cfg.get("password_hash"))
            ...
            try:
                ...
                for row in rows[:MAX_SCAN_ROWS]:
                    ...
                    if not encoded:
                        continue
                    try:
                        config_text = sanitize_openvpn_config(decode_config(encoded))
...
def test_node_by_id(node_id: str) -> dict[str, Any]:
    ...
    try:
        CONFIG_DIR.mkdir(exist_ok=True, parents=True)
        temp_path.write_text(sanitize_openvpn_config(config_text), encoding="utf-8")
    except Exception as e:
...
def connect_node(node_id: str) -> str:
    ...
    try:
        ...
        try:
            CONFIG_DIR.mkdir(exist_ok=True, parents=True)
            config_path.write_text(sanitize_openvpn_config(node.get("config_text") or ""), encoding="utf-8")
        except Exception as e:
        ...
        with lock:
            ...
            for n in merged:
                ...
                if not config_path.exists():
                    try:
                        config_path.write_text(sanitize_openvpn_config(n["config_text"]), encoding="utf-8")
                    except Exception:
                        pass
...
class Handler(BaseHTTPRequestHandler):
    ...
    def is_authorized(self) -> bool:
        ui_cfg = load_ui_config()
        if not (ui_cfg.get("password") or ui_cfg.get("password_hash")):
    ...
    def do_GET(self) -> None:
        ...
        if effective_path == "/sub/clash":
            token = (query.get("token") or [""])[0]
            if not subscription_token_valid(token):
                self.send_json({"error": "Invalid subscription token"}, HTTPStatus.UNAUTHORIZED)
                return
            self.send_bytes(build_clash_subscription(self.headers, query).encode("utf-8"), "text/yaml; charset=utf-8")
            return
        ...
        if effective_path in ("/", "/index.html"):
            ...
            if node and node.get("config_text"):
                self.send_bytes(sanitize_openvpn_config(node["config_text"]).encode("utf-8"), "application/x-openvpn-profile")
            else:
    ...
    def do_POST(self) -> None:
        ...
        if effective_path == "/api/login":
            try:
                client_ip = normalize_client_ip(str(self.client_address[0] if self.client_address else "unknown"))
                locked_for = login_locked_seconds(client_ip)
                if locked_for > 0:
                    self.send_json({"ok": False, "error": f"登录尝试过多，请 {locked_for} 秒后再试"}, HTTPStatus.TOO_MANY_REQUESTS)
                    return
                ...
                ui_cfg = load_ui_config()
                expected_uname = str(ui_cfg.get("username", "admin"))
                username_ok = secrets.compare_digest(input_uname, expected_uname)
                password_ok = verify_ui_password(input_pwd, ui_cfg)
                
                if username_ok and password_ok:
                    record_login_success(client_ip)
                    migrate_legacy_password_if_needed(ui_cfg, input_pwd)
                    ...
                    locked_for = record_login_failure(client_ip)
                    if locked_for > 0:
                        self.send_json({"ok": False, "error": f"登录失败次数过多，已临时锁定 {locked_for} 秒"}, HTTPStatus.TOO_MANY_REQUESTS)
                    else:
        ...
        if effective_path == "/api/update_credentials":
            try:
                ...
                ui_cfg = load_ui_config()
                if not new_username or (not new_password and not (ui_cfg.get("password") or ui_cfg.get("password_hash"))):
                ...
                if new_password:
                    set_ui_password(ui_cfg, new_password)
                save_ui_config(ui_cfg)
                if reauth_required:
                    with lock:
