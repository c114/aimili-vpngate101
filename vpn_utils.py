...
def check_and_fix_dns() -> None:
    ...
    By default this only reports the issue. Set AUTO_FIX_DNS=1 to append
    public DNS nameservers to /etc/resolv.conf on simple VPS installs.
    ...
    if os.environ.get("AUTO_FIX_DNS", "0").lower() not in ("1", "true", "yes", "on"):
        print("[dns_heal] DNS 解析失败但网络可达；默认不修改 /etc/resolv.conf。需要自动修复请设置 AUTO_FIX_DNS=1。", flush=True)
        return
